...
\item 
Assume that $H:\{0, 1\}^{\*}\rightarrow \{0, 1\}^{n}$ is a hash function, where $n\in \mathbb{N}$. Prove that, regardless of the internal structure of $H$, a collision can be found with high probability in $\mathcal{O}(\sqrt{n})$ steps, where $\mathcal{O}(\sqrt{n})$ words of memory are used. 
\end{alphaparts}
\end{numedquestion}
...
where $t\in \mathbb{N}$. 
	\begin{alphaparts}
		...
		\item By the birthday paradox, prove that if we add new rows to the above array, the additional rows have likely no common point with the previous ones as long as $t\cdot mt \leq N$. Hence, choosing $m$ and $t$ such that $mt^{2} > N$ will not be a good choice. 
